...
구현 팁
1. this == o 인지 본다 (주소값 레벨) > 같으면 바로 리턴
2. instanceof로 타입 비교
3. 해당 타입으로 타입캐스팅
4. 값들 중 반드시 비교해야 할 값 비교
  1. primitive type은 ==로 비교
  2. reference type은 equals로 비교
5. 그냥 구글 AutoValue를 써도된다
  - 다만 annotation을 읽어서 컴파일 타임에 추가적으로 클래스를 생성하는 것(롬복처럼 클래스에 메서드 넣어주는것 아님)