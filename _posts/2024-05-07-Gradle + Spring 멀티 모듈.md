...
![](/assets/images/Pasted%20image%2020260228180650_2e6e9951.png)
...
1. 공통모듈 잘 정의하면 소스코드 중복 막을 수 있음.
2. 각 프로젝트마다 인텔리제이를 켜야할 필요 없음(물론 프로젝트 세팅을 하면 되지만 귀찮고 설정한 내용을 가지고 다니기도 그렇고)

## 단점

1. 초기 셋팅이 힘듦
  - 단일 프로젝트로만 개발했기 때문에 처음에 러닝커브가 약간 있을듯.
2. 현재 케이뱅크 CHA-PKG 브랜치 전략에 적합한지는 의문.
3. 영향도 문제
  1. 원래 프로젝트 하나에 파드 하나로 정한 이유는 공통 모듈을 최소화 함으로써, 복붙을 하더라도 다른 파드에 영향을 주지 않게 하기 위해서
  2. 다만 저 당시 논의는 비즈니스 공통로직에 관한 것이기 보다는 행내 시스템 관련 공통로직(EAI 호출 Http메소드, DB커넥션 관련, GUID 채번같은) 에 관한 논의였음
  3. 업무별로 묶는것
