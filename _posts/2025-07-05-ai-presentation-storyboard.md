...
category:
  - 발표자료
---
저는 케이뱅크 혁신서비스개발팀 김민석입니다.
편의서비스에서 공모주 메이트, 식품물가 알림 서비스, 비상장주식 알림 서비스, 투자홈의 상장주식 해외주식을 개발하였습니다. 7월 말에는 투자캘린더 서비스 오픈, 8월말에는 etf와 더 풍부한 정보가 포함된 투자 둘러보기 서비스 등 연말까지 월별로 투자관련 서비스가 하나 하나 오픈될 예정입니다.

### 필요성

케이뱅크 투자 서비스의 차별화는 투자서비스를 제공하지만 ‘증권사’가 아니라는 점에서 출발합니다. 실시간 시세 정보보다는, **투자가 처음이거나 초보인 고객들에게 유용한 정보를 제공**하는 것이 핵심 MVP 방향이라고 판단하였습니다.
하지만 이러한 정보를 **내부 인력이 직접 가공하기에는 인건비 부담이 크고**, 지속적으로 운영하기 어려운 구조입니다.

기존의 사례
1. 식품물가 알림 서비스 리포트 

![이미지](/assets/images/Pasted%20image%2020260227091008.png)

한국농수산식품유통공사에서 현재 데이터를 받아오고 있는 구조, 위와 같은 문서를 포함하여 데이터가 다양하였지만 행 내에서 데이터를 정형화 하는 프로세스가 없었음 > 사업담당자가 계정계포털에서 수기로 해당 데이터를 입력하여 노출하는 방식 > 지속불가능

1. 공모주 메이트, 비상장주식 알림 서비스 - 금감원 공시 자료 분석

![이미지](/assets/images/Pasted%20image%2020260227091013.png)

금융감독원에서 전자공시를 가져올 수는 있지만 일일이 특정 정보를 파싱하여 유익한 정보를 가져올 수 있지만 비즈니스 개발자 한두명이 데이터를 수집, 가공 적재하고 있는 인력 구조상 가용 자원이 없다고 판단하였습니다.
제가 인턴 ~ 1년차 시절에 몇 가지 POC를 진행하였었는데 크롤링 하여 파싱하는데 많은 품이 들고 공시를 올리는 주체마다 사람이 보기에는 비슷해보이지만 파싱하는데 많은 예외케이스(회사마다 업로드한 파일의 인코딩 방식이 다름 같은 포맷의 표인데 숨겨진 태그가 있다거나)가 있기 때문에 한명의 개발자가 지속적으로 유지보수하기에는 한계가 있다고 생각해 사용하지 않고 있습니다. 현재는 다양한 데이터 원천사로부터 데이터를 받아 좀 더 유의미한 데이터를 추려낼 수 있는 방향으로 데이터 수집 파이프라인을 고도화하고 있습니다.    
하지만 외부 데이터 피드 업체에 가공을 의뢰하는 것도 **범위의 한계가 있고, 추가 비용이 지속적으로 발생**한다는 점에서 현실적인 제약이 있습니다.

따라서 여러 외부 데이터를 조합하고, AI를 활용해 **데이터를 비선형적으로 처리하고 요약해주는 자동화 프로세스**를 만든다면, **적은 인력으로도 충분히 경쟁력 있는 정보 제공 서비스를 구현할 수 있을 것**이라는 생각 아래 본 POC를 진행하게 되었습니다.
...
![이미지](/assets/images/Pasted%20image%2020260227091019.png)
...
![이미지](/assets/images/Pasted%20image%2020260227091024.png)

2. 기계적 파싱 및 청킹 수행(위의 이미지에서 4~5)

![이미지](/assets/images/Pasted%20image%2020260227091027.png)

3. 청킹 된 데이터 임베딩 수행

![이미지](/assets/images/Pasted%20image%2020260227091034.png)

4. 실제 비즈니스 요건에 맞춰 응답 결과 생성

![이미지](/assets/images/Pasted%20image%2020260227091037.png)
...
![이미지](/assets/images/Pasted%20image%2020260227091047.png)
...
![이미지](/assets/images/Pasted%20image%2020260227091100.png)
...
위 두 주제는 사용자 경험 개선을 위한 아이디어이고 해당 슬라이드는 생산성 향상 사례라고 보시면 됩니다.
...
이러한 작업은 지난 2년간 전체 개발 리소스 중 최소 7**0% 이상이 소모된 핵심 반복 업무**였으며, 신규 서비스 개발 시마다 병목으로 작용하곤 했습니다.

제가 작년부터 대외에서 데이터를 수집하고 메타 표준에 맞춰 테이블에 저장한 건만 대략 50건 정도 되는 것 같습니다. 크게 세 가지 문제점이 있었습니다.
1. MSA 컨테이너상의 자바 기반 배치 어플리케이션의 느린 배포 속도
2. 대외계 - 퍼블릭망 시 fixedLength - json 매핑 처리의 병목
3. 개발자가 외부 필드를 내부 메타 필드로 매핑해줄 때 병목
1번 2번 문제같은경우는 플랫폼아키텍쳐팀과 IT서비스허브팀분들의 도움을 많이 받아 대부분 해결되었습니다. 자바 기반 배치 어플리케이션의 느린 배포 속도는 데이터 적재 배치를 파이썬 컨테이너로 바꾸면서 컴파일러 기반 > 인터프리터 기반 파일 복사배포로 바뀌면서 배포 속도를 기존 8분에서 1분으로 크게 줄일 수 있었고 fixedLength - json 매핑 처리의 병목 이슈는 연초 아키텍쳐협의회에서 단위업무에서 대외계를 EAI나 카프카를 거치지 않고 직접 호출 할 수 있는 것으로 대부분 해소되었습니다.

하지만 개발자가 직접 외부 필드 명을 내부 메타 표준으로 반복적으로 바꾸는 행위를 계속 해야 했습니다.

![이미지](/assets/images/Pasted%20image%2020260301230554.png)

금주 메타 반영 및 정기배포도 테이블 28건이 신규로 들어갔는데, 위 로직을 28건 짜야 했습니다.
...
![이미지](/assets/images/Pasted%20image%2020260301230601.png)
