...
setter 제공하지 않음
확장 못하게(private으로 멤버가 선언되어도 상속하면서 public으로 바꿀 수 있음)final로 선언
모든 필드를 final로 선언
모든 필드를 private로 선언
private final 뿐만 아니라 내부의 가변 컴포넌트에 접근(레퍼런스를 통한 변경)하지 못하게 함.

### 단점

별도의 객체 일일이 만들어야함. 만드는 과정에서 자원이 많이 필요하거나 제한적인 경우 사용할 수 없다
...
[Record]({% post_url 2023-09-22-Record %})
