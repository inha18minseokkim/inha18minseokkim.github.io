...
바로 완치. 실시간성이 초 단위로 타이트하게 필요한 게 아니라면, 굳이 라는 생각이 들어 수긍하였다. 사실 원래도 정보계가 CDC를 쓰는 사례가 있을테니 나도 그럼 거기에 빨대나 꽂자 생각이었는데 Aurora는 아니었음 (반대로 IDC에서 사용하는 Postgre EDB는 logical level에 커넥터가 있음)
