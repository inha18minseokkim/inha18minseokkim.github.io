...
그렇다고 포스팅할 때마다 직접 `{% raw %}{% post_url %}{% endraw %}` 형식으로 고쳐쓰는 건 번거로움.
...
커밋 직전에 `_posts/` 안의 스테이징된 파일을 스캔해서 `[[파일명]]`을 `{% raw %}{% post_url 파일명 %}{% endraw %}`으로 치환하고 다시 스테이징하는 방식.
...
new_body = new_body.replace('{% post_url ', '{% post_url ').replace(' %}', ' %}')
...
이제 옵시디언에서 `[[파일명]]`으로 링크를 달아도 커밋 시 자동으로 `{% raw %}{% post_url 파일명 %}{% endraw %}`으로 바뀜.
