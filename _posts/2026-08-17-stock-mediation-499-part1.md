---
title: stock-mediation 499 트레이싱 일대기 (1부) — idle 커넥션 가설에서 코루틴까지
date: 2026-08-17
tags:
  - Spring
  - WebFlux
...
결론부터 말하면 이 499는 **eBPF 계측 도구의 구조적 한계로 인한 노이즈**였다. 실제 애플리케이션에는 아무 영향이 없었음. 근데 그 결론까지 오는 데 조사 과정이 꽤 길었고, 중간에 진짜 문제(커넥션 풀 고갈)도 하나 발견했고, SCG까지 방어를 확장하는 과정에서 회귀 버그도 하나 터졌다. 3부로 나눠서 적는다.

- **1부 (이 글)**: idle 커넥션 가설 → SCG 튜닝 → 코루틴 캐스케이드 가설 기각
- [**2부**]({% post_url 2026-08-17-stock-mediation-499-part2 %}): 재시도 계측 → 커넥션 풀 고갈 → Reactor Netty 내부 → 가설들 기각
- [**3부**]({% post_url 2026-08-17-stock-mediation-499-part3 %}): eBPF 최종 원인 확정 → SCG 방어 확장 → 회귀 버그 → 결론
...
```kotlin
// WebClientManager.kt — GET 계열 공통 패턴
fun get(...): Mono<T> =
    webClient.get()
        ...
        .retryWhen(
...
이걸 보고 `InvestingHomeController.kt`를 분석했더니 `CoroutineUtil.kt`의 구조가 눈에 걸렸다.
...
둘째가 결정적이었는데 — 실제 트레이스를 보면 detail/rank 등 **다른 엔드포인트**에서도 똑같이 "정상 호출들 중 딱 하나만 499, 나머지는 전부 정상"이 **100%** 관측된다. 코루틴 캐스케이드 이론이 맞다면 "진짜 원인이 된 실패 호출(에러 상태) + 그로 인해 끊긴 희생자(499)"가 최소 2개는 트레이스에 같이 찍혀야 한다. 근데 실제로는 **원인이 될 만한 실패가 트레이스 어디에도 없이 499 하나만 단독으로** 나온다.
